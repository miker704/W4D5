...
# Sketch out on paper how you'd approach this problem. What would be the running time 
# of your proposed solution? Take as long as you need, but don't write any code yet.

# Brute force
# If we weren't worried about time complexity, what would be the most straightforward 
# way to solve this problem? To start with, we could check every possible pair. 
# If we sum each element with every other, we're sure to either find the pair that 
...
# Make sure that you don't pair an element with itself. However, you don't need to bother checking
#  for summing the same pair twice; that won't affect your result.
...
# the brute-force approach is beneath you. Leave that nonsense to the riffraff. Instead, you'll 
...
# Hash Map
# Finally, it's time to bust out the big guns: a hash map. Remember, a hash map has O(1) 
# #set and O(1) #get, so you can build a hash out of each of the n elements in your array in O(n) time.
...
# Then call over your TA and show them what you've got. Defend to them why each of your solutions has
#  the time complexity you claim it does.
